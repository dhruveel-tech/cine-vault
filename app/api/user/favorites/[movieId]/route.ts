...
export async function POST(_request: Request, context: RouteContext) {
  ...
  const user = await User.findById(result.userId).select("favorites");
  const ids = (user?.favorites ?? []).map((item: any) => item.toString());
  const isActive = ids.includes(movieId);

  await User.updateOne(
    { _id: result.userId },
    isActive ? { $pull: { favorites: movieId } } : { $addToSet: { favorites: movieId } }
  );
...
export async function DELETE(_request: Request, context: RouteContext) {
  ...
  await User.updateOne({ _id: result.userId }, { $pull: { favorites: movieId } });
